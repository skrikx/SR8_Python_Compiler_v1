# SR8 Documentation

SR8 is a local-first intent compiler that converts weakly structured intent into canonical artifacts and derivative outputs.

## Start Here

1. [Getting Started](getting-started.md)
2. [Installation](installation.md)
3. [CLI Reference](cli-reference.md)
4. [Examples](examples.md)

## Core Product Docs

- [Concepts](concepts.md)
- [Artifact Schema](artifact-schema.md)
- [Profiles](profiles.md)
- [Profile Authoring](profile-authoring.md)
- [Transforms](transforms.md)
- [Transform Authoring](transform-authoring.md)
- [Storage and Receipts](storage-and-receipts.md)
- [Diff and Lint](diff-and-lint.md)
- [Python API](python-api.md)
- [Development](development.md)

## Release and Security

- [Release Automation](release-automation.md)
- [Release Process](release.md)
- [Security](security.md)

## OSS Launch and Operations

- [OSS Launch](oss-launch.md)
- [Triage](triage.md)
- [Release Notes Policy](release-notes-policy.md)
- [Post-Launch Ops](post-launch-ops.md)
- [Labels](labels.md)
